...
growth_constrain <- rjson::fromJSON(file = file.path(input.dir, 'control_parameters.json'))$fleet_expansion_limit
...
for(this.year in Year.Range){
  for(truck.type in FCET.types){
    ...
    if(growth_constrain){
      ...
      # fleet.candidates$new.fcet.wanted <- pmin(fleet.candidates$new.fcet.wanted, pmax(50, round(0.1 * fleet.candidates$pre.existing.fcet.pop))) # make sure no fleet grows faster than 10%, unless 50% is fewer than 50
        # If a fleet has a large pre-existing FCET population (>=100), then it can't grow more than `growth_constrain`, which is for example 0.1, meaning 10% constrain
        large.fleets.filter <- fleet.candidates$pre.existing.fcet.pop >= 100
        fleet.candidates$new.fcet.wanted[large.fleets.filter] <- pmin(fleet.candidates$new.fcet.wanted[large.fleets.filter], (fleet.candidates$pre.existing.fcet.pop[large.fleets.filter] * growth_constrain) %>% floor() )
    }
