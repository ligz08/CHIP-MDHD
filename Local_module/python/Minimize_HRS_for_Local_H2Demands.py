...
try:
    ...
    for cy in cyRange:
        ...
        for coverageMinutes in coverageMinutesRange:
            # Set Location Allocation output layer
            out_NA_layer_name = "Local_Min_Facilities_{}_{:02d}min_coverage".format(cy, coverageMinutes)
            # out_NA_layer_path = os.path.join(gdb_path, out_NA_layer_name)

            # Create Location-Allocation layer
            out_NA_layer = arcpy.MakeLocationAllocationLayer_na(
            ...
            subLayerNames = arcpy.na.GetNAClassNames(out_NA_layer)
            print "\tCreated a Location-Allocation Layer named {0}. Sub-layers: {1}.".format(out_NA_layer_name, subLayerNames)
            print datetime.now().time(), "- completed creating Location-Allocation layer."


            # Load the candidate station locations as facilities using default search tolerance and field mappings.
            candidateFieldMappings = arcpy.na.NAClassFieldMappings(out_NA_layer, subLayerNames["Facilities"])
            candidateFieldMappings["Name"].mappedFieldName = "hub.name"
            arcpy.na.AddLocations(
                in_network_analysis_layer = out_NA_layer,
            ...
            # Use the field mappings to map the Weight property from Daily.kgH2 field.
            demandFieldMappings = arcpy.na.NAClassFieldMappings(out_NA_layer,
            ...
            arcpy.na.AddLocations(
                ...
                search_tolerance = "",
                exclude_restricted_elements = "EXCLUDE")
            print "\tAdded locations to the Location-Allocatin Layer."
            ...
            print datetime.now().time(), "- complete solving."

            #Save the solved location-allocation layer as a layer file on disk with relative paths
            # arcpy.management.SaveToLayerFile(out_NA_layer, , "RELATIVE")
            # out_NA_layer.saveACopy(os.path.join(scenario_dir, out_NA_layer_name+".lyrx"))

            facilitiesLayer = arcpy.mapping.ListLayers(out_NA_layer, "Facilities")[0]
            desc = arcpy.Describe(facilitiesLayer)
            arcpy.MakeFeatureLayer_management(in_features = facilitiesLayer,
                                              out_layer = "chosen_facilities_lyr",
                                              where_clause = "FacilityType=3")
            out_shp_filename = os.path.join(out_shapefile_dir, out_NA_layer_name)
            arcpy.CopyFeatures_management("chosen_facilities_lyr",  out_shp_filename)
